...
class PolymorphicTypes extends FunSuite {
  ...
  test("boolIntoIdentity") {
    val program = Application(Lambda("x", new Variable("x")), BoolConst(true))
    Checker.failExpression(program)
  }

  test("lambda") {
    val program = Lambda("x", new Variable("x"))
    Checker.failExpression(program)
  }
  ...
  test("letIdentity") {
    val program = Let("identity", Lambda("x", new Variable("x")), Const(3))
  ...
  test("identitySquareIsNoInt") {
    val identity = Lambda("x", new Variable("x"))
    val identity2 = Lambda("x", new Variable("x"))
  ...
  test("lambdaApplication") {
    val program = Application(Lambda("x", new Variable("x")), Const(3))
    Checker.checkExpression(program)
  }
  ...
  test("lambdaDoesNotGeneralize") {
    val identity = Lambda("x", new Variable("x"))
    val program = Application(Lambda("identity", Application(Application(new Variable("identity"), new Variable("identity")), Const(3))), identity)
  ...
  test("const" ) {
    val const = Lambda("x", Lambda("y", new Variable("x")))
