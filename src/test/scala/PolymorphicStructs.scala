...
import language.types.{BoolLanguageType, IntLanguageType, LanguageTypeVariable, LanguageTypeApplication}
...
class PolymorphicStructs extends FunSuite {
  ...
  test("struct") {
    val structNew = new Binding("newStruct", new New("s", Seq(StructFieldInit("x", Const(3))), Some(IntLanguageType)),
      LanguageTypeApplication(new LanguageStructType("s"), IntLanguageType))
